...
def parse_args():
    ...
    parser.add_argument('--gt-suffix', default='_gt.png',
    ...
    parser.add_argument('--res-suffix', default='_ours.png',
        help='suffix of interpolation result frames'
        )
    parser.add_argument('--motion-mask-dir', default=None,
    ...
    parser.add_argument('--buggy-motion-mask', action='store_true',
        help='whether the provided motion mask is buggy or not')
    args = parser.parse_args()
    return args

def evaluate_single_im(gt_im_path, res_im_path, motion_mask_path=None, 
        mask_out=False, buggy_motion_mask=False):
    ref_im = cv2.imread(gt_im_path)
    res_im = cv2.imread(res_im_path)
    assert np.all(ref_im.shape == res_im.shape), \
    ...
    if motion_mask_path is not None:
        ...
        if buggy_motion_mask:
            ...
            mask_im = (mask_im < 128).astype(np.float32)
        assert np.all(ref_im.shape[:2] == mask_im.shape), \
    ...
    if not mask_out:
        ...
        # idxes = np.where(mask_im > 0.5)
        # mask_im[idxes] = 0
        res_im = res_im.astype(np.float32) * np.tile(mask_im[:,:,np.newaxis], (1, 1, 3))
        ref_im = ref_im.astype(np.float32) * np.tile(mask_im[:,:,np.newaxis], (1, 1, 3))
        err = res_im - ref_im
    ...
    ssim = compare_ssim(res_im, ref_im,
        multichannel=True,
...
def process_single_seq(gt_seq_dir, args):
    ...
    for gt_im_path in gt_im_paths:
        ...
        res_im_path = os.path.join(res_dir,  seq_name,  '01' + res_suffix   )
        #print('----------------------')
        #print(res_dir)
        #print(seq_name)
        #print(im_name[:-len(gt_suffix)] )
        #print(res_suffix)
        #print(im_name[:-len(gt_suffix)] + res_suffix)
        #print('----------------------')

        assert os.path.exists(res_im_path), \
            'Interpolation result {} was not found.'.format(res_im_path)
        if motion_mask_dir is not None:
            mask_im_path = os.path.join(motion_mask_dir,
                seq_name,
                'motion_mask.png'   # problematic, but let's use DVF's naming fashion
                )
        else:
            mask_im_path = None
        m = evaluate_single_im(gt_im_path, res_im_path, mask_im_path, mask_out, buggy_motion_mask)
...
def main(args):
    ...
    print('PSNR: {:.2f}, SSIM: {:.3f}, IE: {:.2f}'.format(
